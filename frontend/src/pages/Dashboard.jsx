...
function SectionLabel({ children }) {
  return (
    <h2 className="mb-3 text-[11px] font-semibold tracking-[0.22em] text-slate-500 uppercase">
      {children}
    </h2>
  )
}
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-8">
      ...
      <section>
        ...
        <ul className="divide-y divide-white/10 rounded-sm border border-white/10 bg-[#10131a]">
          {RECENT_RUNS.map((run) => (
            <li
              key={run.id}
            ...
            >
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">{run.subtitle}</p>
              </div>
              <div className="shrink-0 text-right">
                <StatusBadge tone="online">{run.status}</StatusBadge>
                <p className="mt-1 text-xs text-slate-500">{run.time}</p>
              </div>
            </li>
          ))}
        </ul>
        ...
        <ul className="divide-y divide-white/10 rounded-sm border border-white/10 bg-[#10131a]">
          {INFRASTRUCTURE.map((service) => (
            ...
            >
              ...
              <span className="text-[11px] font-medium tracking-[0.14em] text-emerald-400 uppercase">
                {service.status}
              </span>
            </li>
          ))}
